...
export function analyzeHand(lm: NormalizedLandmark[]): HandSample | null {
  ...
  return { palm, openness, pinch }
}

type HandState = { prevPalm: Vec2; gripping: boolean }
 ...
 * hand so motion is continuous between frames.
 */
export function createHandSlimeDriver(
  ...
  const update = (hands: NormalizedLandmark[][]) => {
    ...
    hands.forEach((lm, i) => {
      ...
      const state: HandState = prev ?? { prevPalm: cur, gripping: false }
